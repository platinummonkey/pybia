...
class PyBiaClient:
    ...
    def _find_pybia_executable(self) -> str:
        ...
        try:
            result = subprocess.run(
                ["which", "pybia"], capture_output=True, text=True, check=True
            )
    ...
    def get_impacted_services(
        ...
        if changed_files:
            ...
            try:
                git_files = (
                    subprocess.run(git_cmd, capture_output=True, text=True, check=True)
                    .stdout.strip()
                    .split("\n")
                )
    ...
    def get_changed_files(self, base_commit: str) -> List[str]:
        ...
        try:
            ...
            changed_files = [
                ...
                if f
                and (f.endswith(".py") or f.endswith(".toml") or f.endswith(".txt"))
            ]
