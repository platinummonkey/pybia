"""
Impact analyzer for pytest-pybia.
...
class ImpactAnalyzer:
    ...
    def analyze_impact(
        ...
        if changed_files:
            ...
            transitive_files = self.dependency_analyzer.get_impacted_files(changed_files)
    ...
    def is_file_impacted(self, file_path: str) -> bool:
        """
        Check if a file is impacted by changes.
        
        Args:
            file_path: Path to the file
            
        Returns:
            True if the file is impacted, False otherwise
        """
        # Normalize path
        ...
        # Check if file is in an impacted service
        for _, service_path in self.impacted_services:
            if file_path.startswith(service_path):
                return True
                
        # Check module
        module_name = self._file_to_module(file_path)
        if module_name and module_name in self.impacted_modules:
            return True
    ...
    def get_impact_summary(self) -> Dict[str, Any]:
        ...
        service_file_counts = {}
        for service_name, service_path in self.impacted_services:
            service_files = [f for f in self.impacted_files if f.startswith(service_path)]
